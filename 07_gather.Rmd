...
This whole section is more or less completely drafted and open to discussion (e.g., what to add/leave out). Some things might need to be explained less technical then.
...
with $\epsilon_j \sim N(0, \sigma)$. Of primary scientific interest here is the parameter $\gamma_4$ describing the effect of governance processes on bare ground cover, conditioned on the other predictors. Note, however, that $\theta_j^{\text{Gov}}$ is not data, but an unobserved characteristic of villages, as described in more detail in section XXX. We hence treat it as a predictor measured with error, which naturally fits into a Bayesian modeling framework. Posterior distributions for $\theta_j^{\text{Gov}}$ were obtained for each village $j$ via IRT modeling (Figure \ref{fig:riestims}) and we denote the obtained posterior mean by $\bar{\theta}_j^{\text{Gov}}$ and the posterior SD by $\bar{\sigma}_j^{\text{Gov}}$. Hence, we can above model equation is extended with the distributional assumption of the latent variable

$$\theta_j^{\text{Gov}} \sim N(\bar{\theta}_j^{\text{Gov}}, \bar{\sigma}_j^{\text{Gov}}).$$
...
Further, since we fixed the scale of the latent village variables in the IRT model to a standard normal distribution, we know that their mean equals zero and their variance equals one (also shown in Table \ref{tab:postpar}). This is supplied as constants to the model. The models are again fit in `brms`, sampling `r summary(fit_bare)$chains * (summary(fit_bare)$iter - summary(fit_bare)$warmup)` posterior draws from `r summary(fit_bare)$chains` chains, after `r summary(fit_bare)$warmup` warm-up iterations per chain. There are no serious convergence issues and posterior predictive checks look fine (Figure \ref{fig:ppds2}).
...
- Agriculture threat BUT CROPLAND INDICATOR > POSITIVE CORRELATION WITH BAREGROUND PERCENT. Also ist beides ein indicator für schlechten boden. Brauche agriculture nicht im DAG.
