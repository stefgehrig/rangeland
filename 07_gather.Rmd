...
\textcolor{red}{add a nice map. Majory:"I imagine this will be used in the final paper so we need to make it a little clearer. Please add an insert map for Tanzania to show where the area of villages in extracted from. Not sure how easy it is in R but its easy in QGIS (free) or ArCGIS if you have access."}
...
```{r boxes, fig.align='center', fig.cap='Rraincloud plots for variables from Table 2. Upper panels (a) and (b) have log-scaled y axes for readability.', out.width="100%"}
...
```{r dagbig, fig.align='center', fig.cap='DAG to illustrate the potential confounding and selection bias affecting estimates of the effect of governance quality on governance outcomes. Circles indicate unobserved variables, square boxes indicate conditioned-on variables due to study design. The color indicates the two variables whose association we analyze. All variables are defined on the village level and indices are dropped for notational brevity.', out.width="90%"}
...
The analyzed outcome in the previous section was based on the synthesis from multiple qualitative data sources that elicited detailed perceptions and interpretations across different actor groups. Arguably, these provide rather accurate judgments of governance outcomes from local observers. These judgments are encoded by the scores in the DF variables of the first-tier category *Outcomes (O)*. As a potential downside, the response scores are "subjective" in the sense that they are self-reported. Therefore, as an alternative measure of commons outcomes, in this section we use remote sensing data from village areas. While such measures can be viewed as "objective", they are more narrow in their definition and are also not free from measurement problems.

The outcome measure is ... . Bare ground is an indicator for low quality of common grazing land. However, bare ground is not per se a suitable 

In particular, the measure we use for commons quality is affected by multiple processes that need to be accounted for when estimating the (conditional) association of quality of governance processes and commons outcomes. We therefore use a causal graph to guide analysis.

Importantly, all potential confounding and selection biases explained in the previous section and laid out in Figure \ref{fig:dagbig} can in the same way affect the relationship between governance and outcomes when using remote sensing data. For simplicity, they are summarized in one single node $U_0$ in the DAG in Figure X. Therefore, we need to keep in mind that also the remote sensing analysis cannot credibly establish a causal chain from variation in governance quality to variation in commons outcomes.

...

For information on cropland cover, we rely on the global data set by @potapov_global_2022, who classified 30m resolution Landsat satellite imagery into presence and absence of cropland. Their data set is split into epochs, two of which (2012–2015 and 2016–2019) well align with the periods before and after the start of the NTRI project. This allows to extract pre- and post-project estimates of cropland area separately for each village in the assessment (data shown in Figure \ref{fig:boxes}; in line with regional and global trends, cropland area increased over time in the studied villages).
...
- wealth inequality from PCA as SD, for example @sharma_escalation_2023. But it REALLY strong correlation with mean/median Welath level from PCA (> 0.75). Just because the presence of rich people increases inequaluty and increases mean wealth. . . Could adjust for it, of course. 
