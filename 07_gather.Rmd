...
header-includes:
  ...
  \vspace{-2.5cm}
  \includegraphics[height=1.15cm]{utils/estimact.png}\\
  \vspace{-0.4cm}
...
```{r boxes, fig.align='center', fig.cap='Raincloud plots for variables from Table 2. Upper panels (a) and (b) have logit-scaled y axes for readability.', out.width="100%"}
...
```{r icc, fig.align='center', fig.cap='Item characteristic curve for one item, as estimated in the ITR model. Expected probabilities for the three ordinal response options are shown (posterior median and 90\\% equal-tailed credible interval) in relation to the underlying latent village-level trait. Upper panel shows the posterior densities of the respective latent trait for all 12 study villages (same color coding as previously).', out.width="70%"}
...
For easier interpretation, we plot model-based predictions for post-period bare ground cover factor change for the 'median village' (all covariables held at their median) as a function of governance quality (Figure \ref{fig:predbare}). This curve is obtained by first back-transforming posterior predictions to the proportion scale and then dividing by pre-period bare ground cover. Judging from the plot, the model roughly predicts increases in bare ground for negative values of the latent variable and decreases in bare ground cover for positive values. But the uncertainty about the effect size remains vast (see credible intervals).

```{r predbare, fig.align='center', fig.cap='Model-based predictions of bare ground cover factor change for the median village for different values of quality of governace processes (posterior median and 90\\% equal-tailed credible interval). Upper panel shows the posterior densities of the respective latent trait for all 12 study villages (same color coding as previously).', out.width="80%"}
include_graphics("outputs/pred_bareground.png")
```

The posterior distributions of the latent governance quality variable for the villages in the sample, as inferred from the IRT model, are shown above the curve. Using this information, we should expect to see both decreases and increases in bare ground in our sampled villages, at least based on the plotted predictions. This is indeed the case in the raw data (Figure \ref{fig:boxes}a).
...
That's why I show a measure -- very similarly constructed as in @sharma_escalation_2023 -- for economic inequality in Figure \ref{fig:boxes}. Indeed, there is some variation.
