...
Given the ordinal nature of the items responses, we use the cumulative logit link with common slopes^[In the IRT terminology, such models are known as "Graded Response Models" [@linden_handbook_1997].]. The probabilities to give responses $k = 1,2,3$ can hence be written as


\begin{align*}
\pi_{1, i, j} & = P(Y_{i,j} = 1) = \text{logit}^{-1}(\beta_1 - \eta_{i,j}), \\
\pi_{2, i, j} & = P(Y_{i,j} = 2) = \text{logit}^{-1}(\beta_2 - \eta_{i,j}) - \text{logit}^{-1}(\beta_1 - \eta_{i,j}), \\
\pi_{3, i, j} & = P(Y_{i,j} = 3) = 1 - \pi_{2, i, j} - \pi_{1, i, j},
\end{align*}

with $\beta_1$, $\beta_2$ being the cutpoint parameters. The parameter $\eta_{i,j}$ is of substantive interest: It is a non-linear combination of village- and item-level characteristics, where higher values of $\eta_{i,j}$ mean a higher expected response category.

Most importantly, $\eta_{i,j}$ shall be affected by $\theta_j^{\text{Gov}}$ and $\theta_j^{\text{Out}}$. These parameters describe the quality of governance processes and of outcomes in village $j$, respectively. Note that here we are ultimately interested in their correlation parameter $\rho_{\text{Gov},\text{Out}}$. Applying the IRT framework, we will model $\eta_{i,j}$ also as a function of easiness of the item, $b_i$. Further, we acknowledge that items do not have constant discrimination. In IRT terminology, this leads to the so-called "two-parameter model" [@linden_handbook_1997], since an additional item-level parameter $a_i$ for the discriminatory power of an item is introduced. The discrimination parameter can be understood as an effect modifier: The stronger discrimination of an item, the more differences in $\theta_j^{\text{Gov}}$ and $\theta_j^{\text{Out}}$ between villages will lead to differences in observed item responses between villages.^[Due to the multiplicativeness of this relationship, the two-parameter model is no more linear in the parameters.] Accordingly, we write


$$\eta_{i,j} = (\alpha + a_i) \left(b_i + \theta_j^{\text{Gov}} \text{Gov}_i + \theta_j^{\text{Out}} \text{Out}_i \right),$$
where $\alpha$ is an intercept parameter for item discrimination, such that $a_i$ represent deviations from mean discrimination for each item. $\text{Gov}_i$ is an indicator that is 1 if item $i$ is of type *Governance* and 0 otherwise. $\text{Out}_i$ is an indicator that is 1 if item $i$ is of type *Outcome* and 0 otherwise.


Finally, working in a Bayesian framework with varying parameters, we assume the following distributions for item- and village-level parameters:
