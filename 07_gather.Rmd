...
The analyzed outcome in the previous section was based on the synthesis from multiple qualitative data sources that elicited detailed perceptions and interpretations across different actor groups. Arguably, these provide rather accurate judgments of governance outcomes from local observers. These judgments are encoded by the scores in the DF variables of the first-tier category *Outcomes (O)*. As a potential downside, the response scores are "subjective" in the sense that they are self-reported. Therefore, as an alternative measure of commons outcomes, in this section we use remote sensing data of village grazing land areas. The outcome measure is area of bare ground and obtained from the MODIS Vegetation Continuous Fields data set with 250 m spatial resolution [@dimiceli_annual_2017]. The data contains annual estimates of percent non-vegetated area. By averaging over the years 2010 to 2015 and 2016 to 2021, we obtain pre- and post-project estimates, respectively, of the area that is bare ground for each village (data shown in Figure \ref{fig:boxes}). 

Bare ground is an indicator for low quality of grazing land and the expansion of bare ground is a threat to local pastoralists [@mapinduzi_use_2003; @wiethase_pathways_2023]. Change in bare ground cover is hence a suitable proxy variable to measure success in commons management i nthe present context. Yet, it is not without caveats. First, all potential confounding and selection biases explained in the previous section and laid out in Figure \ref{fig:dagbig} *can in the same way affect the relationship between governance and outcomes when using remote sensing data*. For simplicity, they are summarized in one single node $U_{123}$ in the DAG in Figure \ref{fig:dagbig2}. Thus, we need to keep in mind that also the remote sensing analysis cannot credibly establish a causal path from variation in governance quality to variation in commons outcomes. Yet, it allows to check if our previous result on the positive association of governance processes and outcomes after the start of the NTRI project still holds when using an alternative, external data source. Second, bare ground cover is affected by other processes that should be accounted for when estimating the (conditional) association of quality of governance processes and commons outcome. A causal graph can guide analysis (Figure \ref{fig:dagbig2}).
...
dag shows: if we want a comparable estimate to previous section (that goal must be mentioned! because u123 kann eh nicht geblockt werden, es eht nur darum zumindest zur selben assoziation zu kommen, die natürlich ganz die selbe ist, weil wir NUR, sttichwort narrow scopr, auf garzing land trends schauen, nicht zB compliance), we should adjust for these two.

a) invasives

Invasive plant species constitute a major disturbance to grazing land health in Northern Tanzania.^[This also why an item on invasive species was elicited in our study as part of the *Outcomes (O)* category (Table \ref{tab:dfsum}).] 
[REF]. Invasive plant species can increase vegetation cover while simultaneously degrading the rangeland. This is a shortcoming.
invasives as mediator but NEGATIVE effect.

invasives: elicited from villagers (third tier outcome variable, see above, used previously)

TNC report: "It should also be noted that decreases in bare ground could be a result of improved rangeland condition or an increase in invasive species."

b) rain

rain very important for abre ground measure [@wiethase_pathways_2023]. Part of U_0 strictly speaking, i.e. U_0 subsumes rain ,  yet we show it. dueto strength of effecta dn ebcuase we can!

The PERSIANN-CCS remote sensing rainfall data was used [@nguyen_chrs_2019] and averaged for years 2016 to 2020, yielding a mean precipitation estimate for each village for the period after project start (data shown in Figure \ref{fig:boxes}).

https://www.nature.com/articles/s41598-023-29358-6:
Heavy, year-round grazing in savannas reduces grazing potential (i.e. quantity of vegetation palatable to grazers) through two pathways that result in either land invaded by toxic and unpalatable plants, including bush encroachment8,17, or in bare ground experiencing soil loss18. Rangeland degradation has been defined as a long-term decline in productivity resulting in rangelands unsuitable for grazing (IPBES19), rather than short-term declines driven by temporal variability of environmental conditions (e.g. rainfall, grazing pressure). Such degradation has been linked to rainfall patterns20, and might be exacerbated by climate-change driven changes in annual rainfall variability, already widely observed across African savannas21. 
Since this study focused on rangeland habitats, we chose degradation parameters from a grazing potential perspective. We followed the Intergovernmental Science-Policy Platform on Biodiversity and Ecosystem Services (IPBES) definition of rangeland degradation, as “persistent loss of vegetation productivity cover, especially of those plants which support herbivores.”19. Our proposed parameters consisted of an index of bare ground cover (absence of grazeable vegetation cover) and the number of individual ITP, which replace plants palatable to herbivores.

<!-- Otelelmet/Ilteremet (Ipomea hildebrandtii), Gugu Karoti (“Carrot Weed”) (Parthenium hysterophorus), Endundulu (Dichrostachys cinerea), and Oldule (Ricinus communis). -->
...
<!-- adjust for pre: total area variation and variation population for example (which affects land use) -->

<!-- aber es gibt ja shcon auch einen vorteil hier oder?! denn alle zeit-konstanten confounder zwischen governance processes und outcomes werden rausgehalten durch das betrachten des deltas (multiplikativ, aufgrund der annahme, dass faktor-veränderung realistischer ist)?! > ne, weil wir ja kein impact assessment machen. es geht ja um faktoren die damit zu tun haben  -->

<!-- ... -->
<!-- Data sources from additional variables in DAG: -->
...
<!-- ------- -->
<!-- 2.	The village polygons overlap the NP.  For the map and the data analysis, we excluded the areas that were within the NP.   -->
<!-- This now includes the raster data: (13.8.23) -->
<!-- Image	Resolution	Values	Source -->
<!-- Crop_pre.tif	30m	1 crop; 0 no crop	https://glad.umd.edu/dataset/croplands -->
<!-- Crop_pre.tif	30m	1 crop; 0 no crop	https://glad.umd.edu/dataset/croplands -->
<!-- Bare_pre.tif	250m	% pixel bare	https://modis.gsfc.nasa.gov/data/dataprod/mod44.php -->
<!-- Bare_post.tif	250m	% pixel bare	https://modis.gsfc.nasa.gov/data/dataprod/mod44.php -->
...
see https://math.stackexchange.com/questions/44771/is-there-an-analogue-to-the-delta-symbol-for-ratios
if I want to use Delta indeed for difference as symbol, but then perceive the log ratio simply as modeling data procession
...
- maybe extend dag: "The DAG starting page 20 on bare ground looks pretty good to me, but I think we’d need a zoom session with Majory and Susan where you explain it, as they are more likely to think of additional paths/variables than I am."
    ...
    - Majory schlägt nochmal das Delta als Outcome vor: "Do you have to add pre- and post-bare or just change in bare ground?"
    
    - Monique daraufhin: "I am not sure I quite understand this bullet point. Do we have data on disturbnaces (demog or other) at a scale that would likely affect before/after comparisons?"
